...
public static class MpqInPlacePatcher
{
    ...
    private const uint CompressedMultiFlag = 0x00000200;
    ...
    /// <summary>
    /// Splits <paramref name="content"/> into MPQ-format sectors, zlib-compressing each one
    /// (falling back to storing that sector raw if compression doesn't actually shrink it),
    /// and prepends the sector-offset table — matching how every other CompressedMulti file
    /// in the archive is already stored.
    /// </summary>
    private static byte[] BuildCompressedFileData(byte[] content, uint sectorSize)
    {
        var sectorCount = content.Length == 0 ? 1 : (content.Length + (int)sectorSize - 1) / (int)sectorSize;
        var sectorBlobs = new List<byte[]>(sectorCount);

        for (var s = 0; s < sectorCount; s++)
        {
            var start = s * (int)sectorSize;
            var len = Math.Min((int)sectorSize, content.Length - start);
            var raw = new byte[len];
            Array.Copy(content, start, raw, 0, len);

            var compressed = ZlibCompress(raw);
            if (compressed.Length + 1 < len)
            {
                var blob = new byte[compressed.Length + 1];
                blob[0] = 0x02; // MPQ compression-method byte: zlib
                Array.Copy(compressed, 0, blob, 1, compressed.Length);
                sectorBlobs.Add(blob);
            }
            else
            {
                sectorBlobs.Add(raw); // compression didn't help — store this sector raw, no method byte
            }
        }

        var offsets = new uint[sectorCount + 1];
        uint cursor = (uint)(4 * (sectorCount + 1));
        offsets[0] = cursor;
        for (var s = 0; s < sectorCount; s++)
        {
            cursor += (uint)sectorBlobs[s].Length;
            offsets[s + 1] = cursor;
        }

        using var ms = new MemoryStream();
        foreach (var o in offsets)
            ms.Write(BitConverter.GetBytes(o));
        foreach (var blob in sectorBlobs)
            ms.Write(blob);
    ...
    public static void Patch(string mapPath, string outputMapPath, IReadOnlyDictionary<string, byte[]> filesToWrite)
    {
        ...
        var sectorSize = 0x200u << ReadU16(headerOffset + 14); // BlockSize field: sector size = 512 << value
        ...
        foreach (var kvp in filesToWrite)
        {
            ...
            var storedBlob = BuildCompressedFileData(content, sectorSize);

            blockEntries[(int)blockIndex] = new BlockEntry
            {
                FileOffset = appendCursor - (uint)headerOffset,
                CompressedSize = (uint)storedBlob.Length,
                FileSize = (uint)content.Length,
                Flags = FileExistsFlag | CompressedMultiFlag,
            };

            appendedChunks.Add(storedBlob);
            appendCursor += (uint)storedBlob.Length;
        }
